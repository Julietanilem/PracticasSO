...
PendSV_Handler:
    ...
    /* If current_task_id is 0, we just switched from task 0 to task 1, so we save the context of task 0 in sp_task1 */
    cmp r3, #0
    beq save_task1

    /* If current task id is 1, we just switched from task 1 to task 2,  so we save the context of task 1 in sp_task2. If it's 2, we save in sp_task3. */  
    cmp r3, #1
    beq save_task2
...
save_task2:
    ...
    movs r3, #2             /* Prepare the ID for the next task (2) */
    b switch_context        /* Jump to the context switch part */
...
switch_context:
    ...
    /* If current_task_id was 0, we need to load task 1's context. */
    cmp r3, #0
    beq load_task1

    /* If it was 1, we load task 2's context. If it was 2, we load task 3's context */
    cmp r3, #1
    beq load_task2
