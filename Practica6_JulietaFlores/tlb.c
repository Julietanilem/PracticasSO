    ...
    if (entry_index == -1) {
        ...
        for (int i = 1; i < TLB_SIZE; i++) {
            ...
            if (tlb[i].last_access < tlb[victim].last_access ||
               (tlb[i].last_access == tlb[victim].last_access &&
                tlb[i].insertion_time < tlb[victim].insertion_time)) {
                victim = i;
            }
        }
        ...
        printf("TLB EVICT: Entry %d (Page %d) discarded for Page %d\n",
            victim, tlb[victim].page, page); // Necesario para pasar tests de eviction
    }
